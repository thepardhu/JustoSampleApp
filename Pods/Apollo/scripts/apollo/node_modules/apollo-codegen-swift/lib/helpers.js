...
class Helpers {
    ...
    fieldTypeEnum(type, structName) {
        if (graphql_1.isNonNullType(type)) {
            return language_1.swift `.nonNull(${this.fieldTypeEnum(type.ofType, structName)})`;
        }
        else if (graphql_1.isListType(type)) {
            return language_1.swift `.list(${this.fieldTypeEnum(type.ofType, structName)})`;
        }
        else if (graphql_1.isScalarType(type)) {
            return language_1.swift `.scalar(${this.typeNameForScalarType(type)}.self)`;
        }
    ...
    enumDotCaseName(name) {
        return language_1.swift `.${language_1.SwiftSource.memberName(camelCase(name))}`;
    }
    ...
    dictionaryLiteralForFieldArguments(args) {
        function expressionFromValue(value) {
            if (value.kind === "Variable") {
                return language_1.swift `GraphQLVariable(${language_1.SwiftSource.string(value.variableName)})`;
            }
            else if (Array.isArray(value)) {
                return language_1.SwiftSource.wrap(language_1.swift `[`, language_1.SwiftSource.join(value.map(expressionFromValue), ", "), language_1.swift `]`);
            }
            else if (typeof value === "object") {
                return language_1.SwiftSource.wrap(language_1.swift `[`, language_1.SwiftSource.join(Object.entries(value).map(([key, value]) => {
                    return language_1.swift `${language_1.SwiftSource.string(key)}: ${expressionFromValue(value)}`;
                }), ", ") || ":", language_1.swift `]`);
            }
        ...
        return language_1.SwiftSource.wrap(language_1.swift `[`, language_1.SwiftSource.join(args.map(arg => {
            return language_1.swift `${language_1.SwiftSource.string(arg.name)}: ${expressionFromValue(arg.value)}`;
        }), ", ") || ":", language_1.swift `]`);
    }
    mapExpressionForType(type, isConditional = false, makeExpression, expression, inputTypeName, outputTypeName) {
        ...
        if (graphql_1.isListType(type)) {
            ...
            if (isOptional) {
                return language_1.swift `${expression}.flatMap { ${makeClosureSignature(this.typeNameFromGraphQLType(type, inputTypeName, false), this.typeNameFromGraphQLType(type, outputTypeName, false))} value.map { ${makeClosureSignature(this.typeNameFromGraphQLType(elementType, inputTypeName), this.typeNameFromGraphQLType(elementType, outputTypeName))} ${this.mapExpressionForType(elementType, undefined, makeExpression, language_1.swift `value`, inputTypeName, outputTypeName)} } }`;
            }
            else {
                return language_1.swift `${expression}.map { ${makeClosureSignature(this.typeNameFromGraphQLType(elementType, inputTypeName), this.typeNameFromGraphQLType(elementType, outputTypeName))} ${this.mapExpressionForType(elementType, undefined, makeExpression, language_1.swift `value`, inputTypeName, outputTypeName)} }`;
            }
        }
        else if (isOptional) {
            return language_1.swift `${expression}.flatMap { ${makeClosureSignature(this.typeNameFromGraphQLType(type, inputTypeName, false), this.typeNameFromGraphQLType(type, outputTypeName, false))} ${makeExpression(language_1.swift `value`)} }`;
        }
        else {
